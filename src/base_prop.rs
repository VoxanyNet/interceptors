...
pub struct BaseProp {
    ...
    /// Have the voxels been modified ever
    pub voxels_modified: bool,
    pub scale: f32,
    pub shader_material: Option<macroquad::material::Material>,
    pub mask: Option<RenderTarget>,
    /// Did the voxels change in the most recently completed tick
    pub voxels_modified_last_tick: bool,
    pub last_tick_removed_voxels: Vec<glamx::IVec2>,
...
impl BaseProp {
    ...
    pub fn inner_tick(
        ...
        if self.removed_voxels != self.last_tick_removed_voxels {
            self.voxels_modified_last_tick = true;
        } else {
            self.voxels_modified_last_tick = false
        }
        self.last_tick_removed_voxels = self.removed_voxels.clone();
    ...
    #[deprecated]
    /// This is a legacy function that just wraps the saves .load() and then downcasts it
    pub fn from_save(
        save: BasePropSave,
        space: &mut Space,
        ...
        save.load(space, textures).downcast_ref::<BaseProp>().unwrap().clone()
    }
    ...
    fn draw_mask(
        ...
        if self.mask.is_none() {
            ...
            let mask = render_target(texture.width() as u32, texture.height() as u32);
